...
# Encoder得到的是网络模型
model = Encoder().cuda(CUDA_NUMBER)
...
def extractor(img, net):
    print(img.shape)
    x = Variable(torch.unsqueeze(img, dim=0).float(), requires_grad=False).cuda(CUDA_NUMBER)
    # y是x的输出
    y = net(x)
    y=y.cpu()
    y = torch.squeeze(y)
...
# data_dir = r'E:/post-graduate/xDNN/image/dir/'
...
# data_dir=r'../VehicleData/train/'
data_dir=r'total/infiltration/'
